...
class EditorViewController: UIViewController {
    ...
    func selectedProcessor() -> Processor {
        if let processorPath = settings.selectedProcessorPath, let processor = settings.enabledProcessors.at(processorPath.row) {
            return processor
        } else {
            settings.selectedProcessorPath = IndexPath(row: 0, section: 0)
            return settings.enabledProcessors.first!
        }
    }
...
//    }
     ...
     */
    func setupProcessor() {
        self.processor = selectedProcessor()

        setupProcessorTitle(self.processor.name)
